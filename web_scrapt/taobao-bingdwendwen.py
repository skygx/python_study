...
def parseArgs():
    ...
    parser.add_argument('--interval', dest='interval', help='抢购商品时查询商品是否可以购买的时间间隔(单位秒)', type=int, required=True)
...
'''淘宝抢购脚本'''
class TaobaoSnap():
    def __init__(self, cookie_cache_name='cookie_cache.pkl', trybuy_interval=1800, server_key=None, **kwargs):
...
if __name__ == '__main__':
    args = parseArgs()
    client = TaobaoSnap(trybuy_interval=args.interval, server_key=args.key)
    client.run()
