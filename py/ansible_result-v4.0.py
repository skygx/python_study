...
2020/8/17  14:14   xguo      2.0         1.实现decide_line
                                               ...
                                               print_excel
                                         2.调试输出
2020/8/19  14:14   xguo      3.0         实现ansible输出单行显示 -o
                                            deal_data_oneline
2020/8/20   17:08  xguo      4.0        把Ansible_Deal类单独存一个文件，import调用
...
class Ansible_Deal:
    ...
    def shift_line(self,data):
        ...
        for i in range(0,len(data)):
            ...
            if self.decide_line(data[i]):
                ...
                # continue
            lines.append(line)
            # mul_lines.append(mul_line)

        for i in numbers[1:]:
           result.append(lines[i - 1])

        result.append(lines[-1])
        return result,mul_lines
    ...
    def save_data(self,host,line,status,code,msg):
        if status in ['SUCCESS','CHANGED']:
            self.output[f"{host}-{line}"]={'hostname':host,'status':status,'code':code,'msg':json.load(msg)}
        else:
            self.output[f"{host}-{line}"]={'hostname':host,'status':status,'code':code,'msg':msg}
    ...
    def deal_data_multiline(self):
        ...
        result=self.shift_line(self.data)
    ...
    def deal_data_oneline(self):
        datas=self.data
        for i in range(0,len(datas)):
            datas[i]=datas[i].replace("=>",'|').replace('>>','|').replace('!:','|')
            msg=datas[i].split('|')
            # print(msg)
            if len(msg) == 3:
                host, sts, message = msg
...
def main():
    ...
    deal=Ansible_Deal('../txt/ansible.log')
    deal.deal_data_multiline()
    # deal.deal_data_oneline()
    # deal.print_output()
